...
**Description:** This routine uses the intermediate value theorem to determine if there is a root on the given interval. If so it can approximate the root by dividing the interval that contains the root in half until a pre-determined maximum number of iterations or tolerance is met. 
...
double bisection (double a, double b, string f, double tol, int maxIter){
    ...
    // call another function to find the values of function at interval end points
    double f_a = func(f, a);
    double f_b = func(f, b);
    double c=0;
    ...
    for (int n=0; n<(log((b-a)/tol)/log(2)+1) && n <maxIter; n++){
        ...
        double f_c= func(f, c);
        // check first to see if the midpoint does equal 0, if yes continue out of the loop
        if (f_c==0){
            continue;
        }
